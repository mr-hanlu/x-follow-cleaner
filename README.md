...
本地导出并筛选 X/Twitter 关注列表，匿名检查最近公开活动，再把确认过的账号分批取消关注。
...
- 断点导出当前账号的完整关注列表
- 匿名请求公开主页，只保留最近可见活动时间等必要字段
- 按未活跃天数、粉丝数、探测状态和审核状态筛选
- 批量标记保留、待取消或已处理
- CSV 备份、进度恢复和多账号数据隔离
...
4. 在“匿名探测公开主页”中设置数量、间隔和并发，先用小批量验证。
...
公开主页通常只提供少量可见内容。`last_post_at` 表示匿名主页响应中可解析到的最新公开状态时间，不代表完整推文历史，也不保证覆盖仅登录可见或受保护内容。
...
- 网页发送的是当前完整的待取消选择。
- 空选择会清空尚未执行的活动队列，但不会删除成功历史。
- 活动队列只保存尚未处理和失败待重试的账号。
- 成功账号会从活动队列移除，并保存成功时间与状态。
- 已成功账号不会因为网页重复发送选择而再次执行。

## 使用建议与已知边界

- 匿名探测建议从并发 `2`、每次 `50` 人开始。
...
- `profile-probe.js`：匿名请求公开主页并提取最近可见活动
