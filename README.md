...
4. 在右下角控制面板粘贴并开始导出；
5. 在“匿名探测公开主页”中设置本次数量和间隔，或选择“处理全部剩余”，分批运行；
...
活动取消队列只保存尚未处理和失败待重试的账号。已经成功取消的账号会从活动队列移除，成功状态与时间保留在账号历史字段中；网页重复发送完整选择时也不会重新加入。网页发送空选择表示清空尚未执行的活动队列，不会删除成功历史。
...
网页把油猴数据视为已同步基线，只用单独的本地脏状态保存尚未发送的修改。发送成功后清除脏状态并采用油猴返回的最新状态，避免旧的浏览器标记覆盖已经完成的取消结果。快速筛选按钮支持再次点击取消，并显示激活状态。
