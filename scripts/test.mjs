...
assert.match(bundle, /@contributionURL\s+https:\/\/clean\.example\.com\/sponsor\//);
assert.match(bundle, /@contributionAmount\s+5 USD/);
...
assert.match(bundle, /<dialog id="xfc-unfollow-confirm"/);
assert.match(bundle, /confirmUnfollowBatch/);
assert.doesNotMatch(bundle, /confirm\(`确认执行最多/);
...
assert.match(bundle, /help\.focus\(\{ preventScroll: true \}\)/);
assert.match(bundle, /id="xfc-probe-delay"[^>]*value="1"/);
assert.match(bundle, /id="xfc-probe-concurrency"[^>]*value="8"/);
assert.match(bundle, /recordSupportMilestone/);
assert.match(bundle, /dismissed_forever/);
assert.match(bundle, /❓ 帮助/);
assert.match(bundle, /怎么复制 Following cURL/);
assert.match(bundle, /@license\s+MIT/);
assert.match(bundle, /@name\s+X\/推特取关助手/);
...
assert.match(bundle, /logBox\.addEventListener\("wheel"/);
assert.match(bundle, /logBox\.scrollTop \+= event\.deltaY \* multiplier/);
...
assert.doesNotMatch(bundle, /__SPONSOR_URL__/);
...
const sponsorHtml = fs.readFileSync(path.join(root, "web", "sponsor", "index.html"), "utf8");
const sponsorSource = fs.readFileSync(path.join(root, "web", "sponsor", "sponsor.js"), "utf8");
...
assert.match(sponsorHtml, /9tguPb7HzyhhhXV8W4MmVv1a2YWkzK7gUG1e3kjpJAC1/);
assert.match(sponsorHtml, /0xc91b1fAF0F82A1CC4EC2Eb9882EDA79a946Ae6D3/);
assert.match(sponsorHtml, /bc1qlvzc7aymxtvxsfagd8uu2yr9f484d7l9hm939j/);
assert.match(sponsorHtml, /src="\.\/alipay\.png"/);
assert.match(sponsorSource, /navigator\.clipboard/);
assert.ok(fs.existsSync(path.join(root, "web", "sponsor", "alipay.png")));
