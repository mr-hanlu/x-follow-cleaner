...
本文档描述 `0.8.3` 的当前实现、数据流、模块边界和后续多语言改造方案。面向后续维护者和接手项目的 AI；普通用户请先阅读 [README.md](./README.md) 上半部分。
...
| `xfc:restore-dataset` | 网页 → 油猴 | 油猴数据为空时，从 CSV 恢复完整数据集 |
| `xfc:restore-saved` | 油猴 → 网页 | 返回恢复账号数和重建队列统计 |
| `xfc:restore-error` | 油猴 → 网页 | 返回来源缺失、账号冲突或数据无效等错误 |

桥接不会把 Cookie、`ct0`、Authorization 或原始 cURL交给网页代码。

CSV 恢复要求每行具有一致且有效的 `source_user_id`，并且油猴当前基础数据为空。恢复时复用分层存储写入逻辑，随后根据 `review_status=remove` 重建待处理队列；已有非空数据不会被 CSV 覆盖。清空当前账号数据时保留活动账号 ID，便于识别旧版缺少来源字段的 CSV，真正取消关注前仍会使用 `twid` 核对登录账号。
